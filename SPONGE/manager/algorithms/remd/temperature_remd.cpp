...
double RequireInputDouble(const ScheduleRecord& schedule, const char* key)
{
    ...
    {
        throw std::runtime_error(
            "TemperatureReplicaExchangePolicy requires schedules.inputs." +
            std::string(key));
    }
    return *value;
}
...
TemperatureReplicaExchangePolicy::TemperatureReplicaExchangePolicy()
    : TemperatureReplicaExchangePolicy(Config{})
{
}
...
std::vector<ExchangePair> TemperatureReplicaExchangePolicy::BuildOddEvenPairs(
...
ExchangeAttempt TemperatureReplicaExchangePolicy::EvaluatePair(
    const ScheduleRecord& left, const ScheduleRecord& right, int exchange_round,
    int pair_index)
{
    if (left.last_observable.step < 0 || right.last_observable.step < 0)
    {
        throw std::runtime_error(
            "TemperatureReplicaExchangePolicy requires valid observables");
    }
    ...
    attempt.log_acceptance =
        (beta_left - beta_right) * (left.last_observable.potential_energy -
                                    right.last_observable.potential_energy);
...
TemperatureReplicaExchangePolicy::AttemptOddEvenRound(Manager* manager,
                                                      int exchange_round)
{
    if (manager == nullptr)
    {
        throw std::runtime_error(
            "TemperatureReplicaExchangePolicy requires a non-null manager");
    }
         ...
         pair_index++)
    {
        ...
        ExchangeAttempt attempt =
            EvaluatePair(left, right, exchange_round, pair_index);
        if (attempt.accepted)
        {
...
TemperatureReplicaExchangePolicy::EpochResult
TemperatureReplicaExchangePolicy::ExecuteEpoch(Manager* manager,
                                               int exchange_round,
                                               bool emit_output)
{
    if (manager == nullptr)
