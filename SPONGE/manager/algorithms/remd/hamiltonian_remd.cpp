...
double ComputeBeta(double temperature_kelvin)
{
    if (temperature_kelvin <= 0.0)
    {
        throw std::runtime_error(
            "HamiltonianReplicaExchangePolicy requires temperature > 0");
    }
...
HamiltonianReplicaExchangePolicy::HamiltonianReplicaExchangePolicy()
    : HamiltonianReplicaExchangePolicy(Config{})
{
}
...
ExchangeAttempt HamiltonianReplicaExchangePolicy::EvaluatePair(
    ...
    int exchange_round, int pair_index)
{
    ...
    {
        throw std::runtime_error(
            "HamiltonianReplicaExchangePolicy requires a non-null manager");
    }
    ...
    if (std::fabs(temperature_left - temperature_right) > 1e-6)
    {
        throw std::runtime_error(
            "HamiltonianReplicaExchangePolicy currently requires equal "
            "temperatures across paired schedules");
    }
    ...
    const double beta = ComputeBeta(temperature_left);
    ...
    attempt.log_acceptance = -beta * ((right_under_left.potential_energy +
                                       left_under_right.potential_energy) -
                                      (left.last_observable.potential_energy +
...
HamiltonianReplicaExchangePolicy::AttemptOddEvenRound(Manager* manager,
                                                      int exchange_round)
{
    if (manager == nullptr)
    {
...
HamiltonianReplicaExchangePolicy::EpochResult
HamiltonianReplicaExchangePolicy::ExecuteEpoch(Manager* manager,
                                               int exchange_round,
                                               bool emit_output)
{
    if (manager == nullptr)
