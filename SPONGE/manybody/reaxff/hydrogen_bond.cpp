    ...
    const int* bond_offset, const int* bond_nbr, const int* bond_idx_arr)
{
    ...
    if (r_ah <= 0.0f || r_ah > 7.5f) return 0.0f;
...
static __global__ void Calculate_HB_Clustered_Gmxpacked(
        ...
        const REAXFF_HB_Entry* hb_entries, int atom_type_numbers,
        ...
        VECTOR* frc, LTMatrix3* atom_virial, float* energy_sum,
        const int* bond_count, const int* bond_offset, const int* bond_nbr,
        const int* bond_idx_arr)
{
    const int hydrogen_index =
        static_cast<int>(blockIdx.x * blockDim.y + threadIdx.y);
    ...
    if (Clustered_Gmxpacked_Center_Cursor_Begin(view, center_cluster, &cursor))
    {
        ...
        while (Clustered_Gmxpacked_Center_Cursor_Next(view, &cursor, &tile))
        {
            ...
            for (int ordinal = static_cast<int>(threadIdx.x);
                 ordinal < neighbor_ordinal_count;
                 ordinal += static_cast<int>(blockDim.x))
            {
    ...
    float atom_en_hb = en_hb;
    Warp_Sum_To(energy_sum, en_hb, warpSize);
    if (atom_energy) Warp_Sum_To(atom_energy + h, atom_en_hb, warpSize);
}
#endif

#ifdef USE_CPU
static void Calculate_HB_Clustered_Native(
                    ...
                    for (int il = 0; il < view.cluster_size; il += 1)
                    {
                        ...
                        for (int jl = 0; jl < view.cluster_size; jl += 1)
                        {
                            if ((view.cluster_valid_masks[cj] & (1u << jl)) ==
                                ...
                                continue;
                            const int atom_j =
                                view.sort_permutation[view.cluster_offsets[cj] +
                                                      jl];
    ...
    const char** failure_reason)
{
    ...
    {
        return fail(
            "clustered ReaxFF hydrogen bond currently requires a single-rank "
...
#ifdef USE_CPU
    if (view.sci_numbers <= 0) return true;
    if (view.sci == NULL || view.cjpacked == NULL ||
        (view.exclusion_pool_numbers > 0 && view.exclusion_mask_pool == NULL))
    {
        return fail(
            "clustered ReaxFF hydrogen bond requires the native pair "
            "payload");
    }
    Calculate_HB_Clustered_Native(
    ...
    if (view.gmxpacked_sci == NULL || view.gmxpacked_cjpacked == NULL ||
        ...
        view.gmxpacked_endpoint_incidence_offsets == NULL ||
        view.gmxpacked_endpoint_incidence_references == NULL)
    {
        return fail(
            "clustered ReaxFF hydrogen bond requires the gmxpacked pair "
            "payload");
    }
    ...
    const dim3 grid((hydrogen_numbers + warps_per_block - 1) / warps_per_block,
                    1, 1);
