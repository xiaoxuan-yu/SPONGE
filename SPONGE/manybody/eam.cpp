...
#ifdef USE_CPU
template <bool full_output>
static void EAM_Clustered_Native_Pairs(
    ...
    LTMatrix3* atom_virial, float* energy_sum, const bool density_stage,
    const bool store_energy, const bool store_virial)
{
    const float cutoff_sq = cutoff * cutoff;
#pragma omp parallel for schedule(dynamic)
    for (int sci = 0; sci < view.sci_numbers; sci += 1)
    {
             ...
             cluster_i += 1)
        {
            ...
            {
                const CLUSTERED_CJ_PACKED packed = view.cjpacked[packed_idx];
                for (int jm = 0; jm < kClusteredJGroupSize; jm += 1)
                {
                    const int cluster_j = packed.cj[jm];
                             ...
                             j_lane += 1)
                        {
                            ...
                            if ((view.cluster_valid_masks[cluster_j] &
                                ...
                                (sci_entry.shift_id ==
                                 ...
                                 cluster_i == cluster_j && j_lane <= i_lane))
                            {
                                continue;
                            }
                            const int sorted_j =
                            ...
                            const int type_j = atom_types[atom_j];
                            const VECTOR drij =
                                (sorted_crd[sorted_i] - sorted_crd[sorted_j]) +
                                shift;
                            const float rij_sq = drij * drij;
                            if (rij_sq <= 0.0f || rij_sq >= cutoff_sq) continue;
                            const float rij = sqrtf(rij_sq);
                            if (density_stage)
                            {
                                atomicAdd(rho + atom_i,
                                          EAM_Interpolate_Value_And_Derivative(
                                              ...
                                              table_dr, rij)
                                              .value);
                                atomicAdd(rho + atom_j,
                                          EAM_Interpolate_Value_And_Derivative(
                                              rho_table + type_i * nr, nr,
                                              table_dr, rij)
                                              .value);
                                continue;
                            }
    ...
    const char** failure_reason)
{
    ...
    {
        return fail("clustered EAM requires the native pair payload");
    }
    EAM_Clustered_Native_Pairs<false>(
        view, view.sort_permutation, d_clustered_sorted_crd, d_atom_type,
        d_electron_density, d_pair_potential, atom_type_numbers, nr, dr, cut,
        cell, d_rho, d_df_drho, frc, atom_energy, atom_virial, d_energy_sum,
        true, false, false);
#else
    ...
    Launch_Device_Kernel(
        ...
        d_atom_type, d_electron_density, nr, dr, cut, cell, d_rho);
#endif
    ...
    else
    {
        EAM_Clustered_Native_Pairs<false>(
            ...
            d_electron_density, d_pair_potential, atom_type_numbers, nr, dr,
            cut, cell, d_rho, d_df_drho, frc, atom_energy, atom_virial,
            d_energy_sum, false, false, false);
    }
#else
