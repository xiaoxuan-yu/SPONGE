...
def make_v2_spec(tmp_path):
    ...
    implementations["baseline"]["sponge_lj_modes"] = {
        "dna_cou": {"force-only": DNA_FORCE_BASELINE_LJ_MODE}
    }
    ...
    [
        ...
        (
            lambda raw: raw["implementations"]["current"].update(
                {
                    "sponge_lj_modes": {
                        "wat160k": {"force-only": DNA_FORCE_BASELINE_LJ_MODE}
                    }
                }
            ),
...
def test_replay_uses_resolved_mode_arguments_and_environment(
    ...
    row = run_replay(
        ...
        snapshot=snapshot,
        sponge_lj_mode=DNA_FORCE_BASELINE_LJ_MODE,
...
def test_v2_dry_run_records_resolved_provenance(tmp_path):
    ...
    assert baseline_dna_force["sponge_lj_mode"] == DNA_FORCE_BASELINE_LJ_MODE
