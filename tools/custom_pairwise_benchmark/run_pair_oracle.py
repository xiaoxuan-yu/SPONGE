...
LOCKED_CLUSTERED_ENV = {
    "SPONGE_CUSTOM_PAIRWISE_CLUSTERED_NATIVE": "1",
...
def parse_args():
    ...
    parser.add_argument("--sponge", type=Path, required=True)
...
def main():
    ...
    sponge = args.sponge.resolve()
    microbench = args.microbench.resolve()
    validate_case(case)
    if not sponge.is_file() or not microbench.is_file():
        raise RuntimeError("SPONGE and NBNXM_MICROBENCH must be existing files")
    ...
    with tempfile.TemporaryDirectory(
        ...
        env = os.environ.copy()
        env.update(LOCKED_CLUSTERED_ENV)
        env["SPONGE_CLUSTERED_DUMP_MICROBENCH"] = str(dump_prefix)
        run_checked(
            [sponge, "mdin.spg.toml"], case, env, args.timeout
        )
        ...
        if not snapshot.is_file():
            raise RuntimeError(
                f"SPONGE completed without writing snapshot: {snapshot}"
            )
        oracle = run_checked(
            [
                microbench,
