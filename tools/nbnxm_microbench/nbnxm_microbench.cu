...
#include "control.h"
#include "neighbor_list/builder/candidate_builder.h"
#include "neighbor_list/provider/state.h"
...
Arguments ParseArguments(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (flag == "--gather-experiment")
        {
            args.gatherExperiment = true;
        }
    ...
    int sitsAtomEnd = -1, float sitsPwwpFactor = 1.0f)
{
    ...
    auto launchForceOnlySplit = [&]() {
        ...
        if (safe_sci != 0 && useLjComb)
        {
            Nbnxm_Clustered_Lennard_Jones_And_Direct_Coulomb_ForceOnly_Warp_Record_Device<
                ...
                ClusteredRegularLJLayoutMode::DENSE_OFFSET,
    ...
    if (replayMode == ProductionGmxpackedReplayMode::split && !useLjComb)
    {
        reported_sci_work_parts = 4;
        reported_contiguous_sci_work = !full_output_mode;
    }
    else if (full_output_mode && !useLjComb && !shiftVirialMode &&
        replayMode != ProductionGmxpackedReplayMode::split)
    {
    ...
    const char* snapshotLabel, int refreshBlockSize)
{
    ...
    auto launchKernel = [&]() {
        Refresh_Gmxpacked_Pair_Shift_Bits<<<sci_numbers, refreshBlockSize>>>(
            ...
            d_excl, d_pair_shift_bits, d_sci_shift_safe_flags);
    ...
    CheckCuda(cudaEventElapsedTime(&total_ms, start, stop),
              "cudaEventElapsedTime");
    std::vector<int> sci_shift_safe_flags(snapshot.sci.size());
    CheckCuda(cudaMemcpy(sci_shift_safe_flags.data(), d_sci_shift_safe_flags,
                         sizeof(int) * sci_shift_safe_flags.size(),
                         cudaMemcpyDeviceToHost),
              "cudaMemcpy(sci_shift_safe_flags)");
    const int safe_count = static_cast<int>(std::count(
        sci_shift_safe_flags.begin(), sci_shift_safe_flags.end(), 1));
    std::printf(
    ...
    const char* snapshotLabel, int refreshBlockSize)
{
    ...
    auto launchKernel = [&]() {
        ...
        Launch_Clustered_Gmxpacked_Candidate_Leaf_Root_Child_Task_Build(
            ...
            true, taskCapacity, d_task_counter, d_task_overflow,
    ...
    bool rootChildDeviceCounter)
{
    ...
    if (rootChildTaskQueue)
    {
        ...
        auto launchTaskBuild = [&]() {
            ...
            Launch_Clustered_Gmxpacked_Candidate_Leaf_Root_Child_Task_Build(
                ...
                snapshot.candidate_shift_ids.empty() ? nullptr
                                                     : d_candidate_shift_ids,
                true, rootChildTaskCapacity, d_root_child_task_counter,
    ...
    if (activeCandidateList)
    {
        ...
        Launch_Clustered_Gmxpacked_Candidate_Leaf_Probe(
            ...
            snapshot.candidate_shift_ids.empty() ? nullptr
                                                 : d_candidate_shift_ids,
            !cooperativeTraversal, cooperativeTraversal, false,
    ...
    auto launchKernel = [&]() {
        ...
        if (rootChildDeviceCounter)
        {
            ...
            Launch_Clustered_Gmxpacked_Candidate_Leaf_Root_Child_Device_Counter_Count(
                ...
                snapshot.candidate_shift_ids.empty() ? nullptr
                                                     : d_candidate_shift_ids,
                !cooperativeTraversal, rootChildTaskCapacity,
        ...
        else
        {
            Launch_Clustered_Gmxpacked_Candidate_Leaf_Probe(
                ...
                snapshot.candidate_shift_ids.empty() ? nullptr
                                                     : d_candidate_shift_ids,
                !cooperativeTraversal, cooperativeTraversal,
...
int main(int argc, char** argv)
{
    ...
    if (args.kernel == "sponge")
    {
        ...
        {
            if (args.gatherExperiment)
            {
                return nbnxm_microbench::RunGatherExperiment(
