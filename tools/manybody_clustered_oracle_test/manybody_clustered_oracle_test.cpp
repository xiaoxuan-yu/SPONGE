...
struct Clustered_Fixture
{
    ...
    CLUSTERED_SCI* sci = nullptr;
    CLUSTERED_CJ_PACKED* cjpacked = nullptr;
    unsigned long long* exclusion_masks = nullptr;
    ...
    Clustered_Fixture(const LTMatrix3& rcell)
    {
        ...
        view.lease_epoch = 43;
        view.native_payload_generation = 47;
...
#ifdef USE_CPU
        std::vector<CLUSTERED_SCI> host_sci(2);
        ...
        std::vector<CLUSTERED_CJ_PACKED> host_packed(2);
        for (auto& packed : host_packed)
        {
            for (int jm = 0; jm < kClusteredJGroupSize; jm += 1)
                packed.cj[jm] = -1;
            for (int split = 0; split < kClusteredWarpSplitCount; split += 1)
                for (int index = 0; index < kClusteredJGroupSize *
                                                kClusteredSuperClusterClusters;
                     index += 1)
                    packed.imei[split].excl_ind[index] = -1;
        }
        host_packed[0].cj[0] = 1;
        host_packed[0].imei[0].imask = 1u;
        Clustered_Exclusion_Index_Ref(host_packed[0].imei[0], 0, 0) = 0;
        host_packed[1].cj[0] = 1;
        host_packed[1].imei[0].imask = 2u;

        sci = Device_Copy(host_sci);
        cjpacked = Device_Copy(host_packed);
        exclusion_masks =
            Device_Copy(std::vector<unsigned long long>{1ull << 3});
        view.sci_numbers = 2;
        view.cjpacked_numbers = 2;
        view.exclusion_pool_numbers = 1;
        view.sci = sci;
        view.cjpacked = cjpacked;
        view.exclusion_mask_pool = exclusion_masks;
#else
    ...
    ~Clustered_Fixture()
    {
        ...
        Device_Free(&sci);
        Device_Free(&cjpacked);
        Device_Free(&exclusion_masks);
